classdef AugmentedLagrangianAlgorithm
    ...
    methods (Access = public)
        ...
        function obj = AugmentedLagrangianAlgorithm()
            ...
            w0                              = c;
            [dc,dc_u,dc_sig,dLa,dC,dU,dSig] = obj.computeParametersGradient(s0,s_ini,sec,l_u,l_sig,rho_u,rho_sig,c_u,c_sig,method,update,dC,dU,dSig);
            [cost, const_u, const_sig, lu, lsig, tauV] = obj.computeMonitoring(c,c_u,c_sig,l_u,l_sig,tau,w0,iter,s0,...
            ...
            while dS > tol || c_u > 0 || c_sig > 0
                ...
                [tau, c_u, c_sig, s]   = obj.determineStepLength(s0,l_u,l_sig,rho_u,rho_sig,tau,LaNew,dLa,cNew);
                [l_u, l_sig]           = obj.updateMultipliers(l_u, l_sig, rho_u, rho_sig, c_u, c_sig);
                [c,c_u,c_sig,LaTrial]  = obj.computeParameters(s,l_u,l_sig,rho_u,rho_sig);
                [dc,dc_u,dc_sig,dLa,dC,dU,dSig] = obj.computeParametersGradient(s,s_ini,sec,l_u,l_sig,rho_u,rho_sig,c_u,c_sig,method,update,dC,dU,dSig);
                [cost, const_u, const_sig, lu, lsig, tauV] = obj.computeMonitoring(c,c_u,c_sig,l_u,l_sig,tau,w0,iter,s,...
                    cost, const_u, const_sig, lu, lsig, tauV, monitor,dc,dc_u,dc_sig);
    ...
    methods (Access = private)
        ...
        function [dc,dc_u,dc_sig,dLa,dC,dU,dSig] = computeParametersGradient(obj,s,s0,sec,l_u,l_sig,rho_u,rho_sig,c_u,c_sig,method,update,dC,dU,dSig)
            switch method
                case 'approach'
                ...
                case 'finite differences'
                    if update == 1
                        [dc, dc_u, dc_sig,dC,dU,dSig] = obj.updateFiniteDiffDerivatives(s,s0,sec);
                    else
                        [dc, dc_u, dc_sig] = obj.computeFiniteDiffDerivatives(s,s0,sec,dC,dU,dSig);
                    end
                otherwise
        ...
        function [cost, const_u, const_sig, lu, lsig, tauV] = computeMonitoring(obj,c, c_u, c_sig, l_u, l_sig, tau, w0, iter, s,...
                cost, const_u, const_sig, lu, lsig, tauV, monitor,dc,dc_u,dc_sig)
            cost(iter+1)      = c;
            const_u(iter+1)   = c_u;
